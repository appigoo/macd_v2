...
# 新增：使用 time.sleep 實現自動刷新（警告：會阻塞 UI，建議僅測試）
if enable_auto_refresh and auto_interval_minutes > 0:
    interval_seconds = auto_interval_minutes * 60
    st.warning(f"自動刷新已啟用，使用 time.sleep 模式（間隔 {auto_interval_minutes} 分鐘）。UI 將被阻塞，請僅用於測試。")
    while True:
        time.sleep(interval_seconds)
        st.rerun()  # 喚醒後 rerun 更新數據
else:
    st.info("自動刷新未啟用。")
...
if enable_auto_refresh:
    st.sidebar.success(f'自動刷新已啟用，每 {auto_interval_minutes} 分鐘一次（sleep 模式）。')
